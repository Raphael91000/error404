...
vec3 palette(float t) {
    // Dark green to bright cyan-green
    vec3 darkGreen = vec3(0.0, 0.3, 0.2);    // Deep tech green
    vec3 midGreen = vec3(0.0, 0.8, 0.5);     // Matrix green
    vec3 brightGreen = vec3(0.2, 1.0, 0.7);  // Bright cyan-green
    
    float wave = 0.5 + 0.5 * sin(t);
    
    // Mix between colors for smooth gradient
    if (wave < 0.5) {
        return mix(darkGreen, midGreen, wave * 2.0);
    } else {
        return mix(midGreen, brightGreen, (wave - 0.5) * 2.0);
    }
}
...
void main() {
    ...
    vec3 bg = mix(vec3(0.0, 0.02, 0.01), vec3(1.0), uTheme);
